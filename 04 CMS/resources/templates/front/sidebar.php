                <div class="col-lg-4">
                    <!-- Search widget-->
                    <div class="card mb-4">
                        <div class="card-header">BUSCAR</div>
                        <div class="card-body">
                            <div class="input-group">
                                <form action="search.php" method="get">
                                    <div class="input-group">
                                        <input class="form-control" type="text" placeholder="Ingrese el título a buscar" name="noti_titulo" required />
                                        <button class="btn btn-primary" id="button-search">Buscar</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                    <!-- CREDENCIALES -->
                    <div class="card mb-4">
                        <div class="card-body">
                            <?php
                                if(isset($_SESSION['user_rol'])){
                                    ?>
                                        <div>
                                            Bienvenido(a), <?php echo $_SESSION['user_nombres'] . " " . $_SESSION['user_apellidos']; ?>
                                            <div>
                                                <a href="logout.php" class="btn btn-warning">Cerrar Sesión</a>
                                            </div>
                                        </div>
                                <?php } else {
                                    ?>
                                        <div>
                                            <a href="login.php" class="btn btn-primary">Iniciar Sesión</a>
                                            <a href="register.php" class="btn btn-success">Registrate</a>
                                        </div>
                                <?php }
                            ?>
                        </div>
                    </div>
                    <!-- Categories widget-->
                    <div class="card mb-4">
                        <div class="card-header">Categories</div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-sm-6">
                                    <ul class="list-unstyled mb-0">
                                        <li><a href="#!">Web Design</a></li>
                                        <li><a href="#!">HTML</a></li>
                                        <li><a href="#!">Freebies</a></li>
                                    </ul>
                                </div>
                                <div class="col-sm-6">
                                    <ul class="list-unstyled mb-0">
                                        <li><a href="#!">JavaScript</a></li>
                                        <li><a href="#!">CSS</a></li>
                                        <li><a href="#!">Tutorials</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- Side widget-->
                    <div class="card mb-4">
                        <div class="card-header">Side Widget</div>
                        <div class="card-body">You can put anything you want inside of these side widgets. They are easy to use, and feature the Bootstrap 5 card component!</div>
                    </div>
                </div>